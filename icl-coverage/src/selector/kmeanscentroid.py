...
class KmeansCentroidScoreSelectorArgs(CommonSelectorArgs):
    def get_name(self):
        return 'Computing Kmeans-centroid'
    

class KmeansCentroidScoreSelector(BaseExampleSelector, SelectorUtilsMixin, BaseModel):
    args: KmeansCentroidScoreSelectorArgs
    ...
    def from_examples(
        ...
        kmeans = KMeans(n_clusters=18, random_state=0, n_init="auto").fit(X_new)

        preds=kmeans.predict(Y_new)
        ...
        for j in tqdm(range(len(Y_new))):
            ...
            for i in range(len(train_labels)):
                if train_labels[i]==preds[j]: #change to be the same as the Y_new value
                    # print("Cluster: {}".format(train_labels[i]))
                    # print(i)
                    cluster_to_orig_map[len(points_in_cluster)]=i
                    indices_in_cluster.append(i)
                    points_in_cluster.append(X_new[i])
            

            if len(points_in_cluster)<5:
                
                    missing_distances = np.linalg.norm((X_new - Y_new[j]), axis=1)
                    missing_sorted_dist=np.argsort(missing_distances)
        
                    for val in missing_sorted_dist:
                        # print(val)
                        if val not in indices_in_cluster:
                            cluster_to_orig_map[len(points_in_cluster)]=val
                            points_in_cluster.append(X_new[val])
            ...
            distances = np.linalg.norm((points_in_cluster - kmeans.cluster_centers_[preds[j]]), axis=1)
            sorted_distances_vals.append(np.sort(distances)[0:5])
            sorted_dist=np.argsort(distances)[0:5]
            # print(sorted_dist)
            top_8_in_cluster=[]
            #print(cluster_to_orig_map)
            for val in sorted_dist:
                top_8_in_cluster.append(cluster_to_orig_map[val])
            sorted_distance_labels.append(top_8_in_cluster)
...
if __name__=='__main__':
    ...
    args = KmeansCentroidScoreSelectorArgs(selector_type=ES.INFLUENCE,n_shots=8)
    #print(args)
    bs_selector = KmeansCentroidScoreSelector.from_examples(args, candidates, example_template, query_examples=test, device=0)
