...
class KmeansClosestScoreSelectorArgs(CommonSelectorArgs):
    def get_name(self):
        return 'Computing Kmeans-centroid'
    

class KmeansClosestScoreSelector(BaseExampleSelector, SelectorUtilsMixin, BaseModel):
    args: KmeansClosestScoreSelectorArgs
    ...
    def from_examples(
        ...
        for j in tqdm(range(len(Y_new))):
            ...
            distances = np.linalg.norm((points_in_cluster - Y_new[j]), axis=1)
        ...
        for idx in query_iter:
            try:
                ...
                print(e)
                print(idx)
                print("Length if sorted distance labels {}".format(len(sorted_distance_labels)))
...
if __name__=='__main__':
    ...
    args = KmeansClosestScoreSelectorArgs(selector_type=ES.INFLUENCE,n_shots=8)
    #print(args)
    bs_selector = KmeansClosestScoreSelector.from_examples(args, candidates, example_template, query_examples=test, device=0)
